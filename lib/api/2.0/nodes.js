...
var nodesMasterDelTagById = controller({success: 204}, function(req) {
    // Workaround for ODR 838: fail to delete tag named by special characters
    // In some case, swagger didn't handle URL path decodeing correctly, this
    // maybe a swagger issue , but still need figure out the root cause.
    return nodes.masterDelTagById(decodeURIComponent(req.swagger.params.tagName.value));
});
